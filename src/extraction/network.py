...
fichier GraphML dans data_raw. Ici on charge ce fichier et on projette le
...
def load_walk_graph(config, logger=None):
    ...
    path = os.path.join(
        ...
        config["chemins"]["fichiers_osm"]["graphe_pieton"],
    )
    ...
    graph = ox.project_graph(graph, to_crs=config["crs_cible"])
