# Objectif, verifier l'agregation de la demande, l'assortiment d'ajout de services, la
# borne superieure du gain et la cote des terrains candidats, sur des donnees synthetiques
# aux reponses connues.
...
from src.processing.service_addition import (
    _initial_coverage,
    build_service_assortment,
    demand_by_node,
    optimal_addition_bound,
)
from src.processing.site_scoring import candidate_type_distances
...
def test_demand_by_node_groups_the_same_node_and_the_same_snap_gap():
    """Deux residences du meme noeud et de la meme classe d'ecart forment un seul point.

    Elles ont alors exactement les memes distances vers tout site candidat, les additionner ne
    change donc aucun resultat de la couverture maximale.
    """
    residences = pd.DataFrame(
        {
            "residence_id": [0, 1, 2],
            "node": ["n1", "n1", "n2"],
            "snap_m": [10.0, 20.0, 5.0],
            "seniors_weight": [3.0, 4.0, 5.0],
            "rest_weight": [1.0, 1.0, 2.0],
        }
    )
    distances = {"epicerie": {0: 250.0, 1: 250.0, 2: 900.0}}
    table, group_distances = demand_by_node(
        residences, distances, ("seniors_weight", "rest_weight"), 25.0
    )
    # Les deux residences du noeud n1 tombent dans la meme classe de 25 m.
    assert list(table["node"]) == ["n1", "n2"]
    assert list(table["seniors_weight"]) == [7.0, 5.0]
    assert table["seniors_weight"].sum() == residences["seniors_weight"].sum()
    # L'ecart du groupe est la moyenne de ses residences.
    assert list(table["snap_m"]) == [15.0, 5.0]
    assert list(group_distances["epicerie"].values()) == [250.0, 900.0]


def test_demand_by_node_separates_two_snap_classes_on_one_node():
    """Deux residences du meme noeud aux ecarts tres differents restent deux points.

    C'est la raison d'etre de la classe d'ecart. Sur un meme noeud, l'ecart va parfois de
    quelques metres a plus d'un kilometre, les confondre ferait heriter la maison eloignee de
    l'accessibilite de la maison proche.
    """
    residences = pd.DataFrame(
        {
            "residence_id": [0, 1],
            "node": ["n1", "n1"],
            "snap_m": [10.0, 900.0],
            "seniors_weight": [3.0, 4.0],
        }
    )
    distances = {"epicerie": {0: 250.0, 1: 1150.0}}
    table, _ = demand_by_node(residences, distances, ("seniors_weight",), 25.0)
    assert len(table) == 2
    assert table["seniors_weight"].sum() == 7.0
