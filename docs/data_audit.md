...
- **Attributs nécessaires présents** : `IDUGD` (aires de diffusion), `MUS_NM_MUN` (limites municipales), `stop_lat`/`stop_lon` (GTFS), `nom_gare`/`nom_train` (exo), `service_type` (services essentiels, champ dérivé du typage).
